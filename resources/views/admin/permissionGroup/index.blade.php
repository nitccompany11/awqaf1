...
@section('content')
	...
	<div class="col-md-12 text-center content-t card">
		<div class="col-md-12 text-right btnDiv"> <!-- Heading Button -->
			...
			<form style="display: inline-block;" method="post" action="{{ url('admin/permissionGroup/deleteAll') }}" id="formDeleteAll">
				{{ csrf_field() }}
		...
		<div class="col-md-12 contentTable table-responsive"> <!-- Content -->
			...
			<table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                ...
                <tbody>
                	...
                	@foreach($permissionGroup as $s)
	                <tr>
	                	<td>
	                		<input type="checkbox" class="inputUser" name="group[]" value="{{ $s->id }}" form="formDeleteAll">
	                	</td>
	                  	<td>{{ $s->id }}</td>
	                  	<td>{{ $s['name_' . getLang()] }}</td>
	                  	<td>{{ $s->created_at->format('Y-m-d') }}</td>
	                  	<td>
		                  	<a href="{{ url('admin/permissionGroup') . '/' . $s->id . '/show' }}" class="btn btn-warning"><i class="fa fa-eye"></i> {{ trans('admin.show') }} </a>

		                  	<a href="{{ url('admin/permissionGroup') . '/' . $s->id . '/permission' }}" class="btn btn-success"><i class="fa fa-key"></i> {{ trans('admin.permission') }} </a>

		                  	<a href="{{ url('admin/permissionGroup') . '/' . $s->id . '/edit' }}" class="btn btn-info"><i class="fa fa-edit"></i> {{ trans('admin.edit') }} </a>
		                  	
		                  	<a href="{{ url('admin/permissionGroup') . '/' . $s->id . '/delete' }}" class="btn btn-danger delete"><i class="fa fa-close"></i> {{ trans('admin.delete') }} </a>
	                  	</td>
	                </tr>
	                @endforeach
	                @else 
	                	<p class="alert alert-warning" style="margin-top: 10px; margin-bottom: 10px"> {{ trans('admin.noData') }} {{ trans('admin.willAdd') }} <a href="{{ url('admin/permissionGroup/add') }}"> {{ trans('admin.addPermissionGroup') }} </a> </p>
	                @endif	
                </tbody>
             </table>
