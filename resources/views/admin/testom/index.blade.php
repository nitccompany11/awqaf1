...
@section('content')
	...
	<div class="col-md-12 text-center content-t card">
		<div class="col-md-12 text-right btnDiv"> <!-- Heading Button -->
			...
			<form style="display: inline-block;" method="post" action="{{ url('admin/testom/deleteAll') }}" id="formDeleteAll">
				{{ csrf_field() }}
		...
		<div class="col-md-12 contentTable table-responsive"> <!-- Content -->
			...
			<table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                ...
                <tbody>
	                ...
	                <tr>
	                	<td>
	                		<input type="checkbox" class="inputUser" name="testom[]" value="{{ $s->id }}" form="formDeleteAll">
	                	</td>
	                  	<td>{{ $s->id }}</td>
	                  	<td>{{ $s['name'] }}</td>
	                  	<td>{{ $s->created_at->format('Y-m-d') }}</td>
	                  	<td>
		                  	<a href="{{ url('admin/testom') . '/' . $s->id . '/show' }}" class="btn btn-warning"><i class="fa fa-eye"></i> {{ trans('admin.show') }} </a>

		                  	<a href="{{ url('admin/testom') . '/' . $s->id . '/edit' }}" class="btn btn-info"><i class="fa fa-edit"></i> {{ trans('admin.edit') }} </a>
		                  	
		                  	<a href="{{ url('admin/testom') . '/' . $s->id . '/delete' }}" class="btn btn-danger delete"><i class="fa fa-close"></i> {{ trans('admin.delete') }} </a>

		                  	@if($s->status == 1)
		                  		<a href="{{ url('admin/testom') . '/' . $s->id . '/status' }}" class="btn btn-danger">
		                  			<i class="fa fa-close"></i>
		                  		</a>
		                  	@else
		                  		<a href="{{ url('admin/testom') . '/' . $s->id . '/status' }}" class="btn btn-success">
	                ...
	                @endforeach
	                @else 
	                	<p class="alert alert-warning" style="margin-top: 10px; margin-bottom: 10px"> {{ trans('admin.noData') }} {{ trans('admin.willAdd') }} <a href="{{ url('admin/testom/add') }}"> {{ trans('admin.addTestom') }} </a> </p>
	                @endif	
                </tbody>
             </table>
