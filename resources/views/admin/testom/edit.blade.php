	...
	<div class="col-md-12 text-center content-t card">
		<div class="form card-body pb-2">
			...
			<form class="form-horizontal" method="post" action="{{ url()->current() }}" enctype="multipart/form-data">
				...
				<input type="hidden" name="id" value="{{ $testom->id }}">

				<div class="row row-sm"> <!-- Item -->
					<label class="label label-control col-md-3"> {{ trans('admin.name') }} </label>
					<div class="col-md-9">
						<input type="text" name="name" class="form-control" placeholder="{{ trans('admin.name') }}" value="{{ $testom->name }}">
					</div>
				</div> <!-- End Item -->
				...
				<div class="row row-sm"> <!-- Item -->
					...
					<div class="col-md-9">
						<input type="text" name="job" class="form-control" placeholder="{{ trans('admin.job') }}" value="{{ $testom->job }}">
					</div>
				</div> <!-- End Item -->
				...
				<div class="row row-sm"> <!-- Item -->
					...
					<div class="col-md-9">
						<textarea class="form-control" placeholder="{{ trans('admin.comment_ar') }}" name="comment_ar">{{ $testom->comment_ar }}</textarea>
					</div>
				</div> <!-- End Item -->
				...
				<div class="row row-sm"> <!-- Item -->
					...
					<div class="col-md-9">
						<textarea class="form-control" placeholder="{{ trans('admin.comment_en') }}" name="comment_en">{{ $testom->comment_en }}</textarea>
					</div>
				</div> <!-- End Item -->
