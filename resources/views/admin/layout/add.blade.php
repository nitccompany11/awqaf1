...
@section('content')
	...
	@section('bredcrumb')
        {{ Breadcrumbs::render('mainbreda', $title, url()->current(), trans('admin.layouts'), substr(url()->current(), 0, -4)) }}
    @endsection
	...
	<div class="col-md-12 text-center content-t card">
		<div class="form card-body pb-2">
			...
			<form class="form-horizontal" method="post" action="{{ url()->current() }}" enctype="multipart/form-data">
				...
				<div class="row row-sm"> <!-- Item -->
					<label class="label label-control col-md-3"> {{ trans('admin.slug') }} </label>
					<div class="col-md-9">
						<input type="text" name="slug" class="form-control" placeholder="{{ trans('admin.slug') }}">
					</div>
				</div> <!-- End Item -->
				...
				<div class="row row-sm"> <!-- Item -->
					...
					<div class="col-md-9">
						<select class="form-control" id="typeLayouts">
							<option>{{ trans('admin.choose') }}</option>
							<option value="1">{{ trans('admin.content') }}</option>
							<option value="2">{{ trans('admin.image') }}</option>
						</select>
					</div>
				</div> <!-- End Item -->

				<div class="row row-sm hidden imageLayouts"> <!-- Item -->
				...
				<div class="row row-sm hidden contentLayouts"> <!-- Item -->
					<label class="label label-control col-md-3"> {{ trans('admin.content_ar') }} </label>
					<div class="col-md-9">
						<textarea class="form-control ckeditor" name="content_ar" placeholder="{{ trans('admin.content_ar') }}"></textarea>
					</div>
				</div> <!-- End Item -->
				...
				<div class="row row-sm hidden contentLayouts"> <!-- Item -->
					...
					<div class="col-md-9">
						<textarea class="form-control ckeditor" name="content_en" placeholder="{{ trans('admin.content_en') }}"></textarea>
					</div>
				</div> <!-- End Item -->
